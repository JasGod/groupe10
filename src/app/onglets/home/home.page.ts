...
export class HomePage implements AfterViewInit {
  ...
  ionViewWillEnter() {
    ...
    this.popinaServ.getMagasin().subscribe((data) => {
      this.magasins = data;
      this.popinaServ.magasins = data;
  ...
  showNOtif() {
    if (this.popinaServ.magasins.every((produits) => produits.stock != 0)) {
      this.localNotif.schedule({
        id: 1,
        text: 'Le stock des produits est renouvellé.',
        sound: 'assets/IntroMamaz.mp3',
      });
      this.presentToast();
    }
  }
  ...
  async presentToast() {
    const toast = await this.toastController.create({
      message: 'Le stock des produits est renouvellé.',
      duration: 2000,
      animated: true,
      position: 'top',
    });
